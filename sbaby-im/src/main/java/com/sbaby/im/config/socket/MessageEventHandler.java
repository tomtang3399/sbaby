...
import com.sbaby.im.constants.SocketMsgType;
...
public class MessageEventHandler extends SbabyCommonService {
	...
	public void onEvent(SocketIOClient client, AckRequest request, SocketMessageInfo data) {
		...
		//判断是否是系统用户发送的消息
		String sourceClientId = data.getSourceClientId();
		if (checkCacheClientExist(sourceClientId) && checkCacheClientOnLine(sourceClientId)) {
			//获取目标客户端编号
			...
			if (checkCacheClientExist(targetClientId)) {
				...
				if (checkCacheClientOnLine(targetClientId)) {
					...
					//离线则异步存入持久层数据库（可用ES搜索引擎数据库/mongodb数据库），状态为未推送状态
